...
class PolicyScheduler:
    ...
    def get_action_and_value(self, obs, policy_type:list[str]):
        ...
        # for i, policy_name in enumerate(policy_type):
        #     if policy_name == "octo":
        #         predicted_actions[i] = self.octo.act(obs["rgb"][i])
        # if "octo" in policy_type:
        #     _, predicted_logprobs, _, predicted_values = self.internal_policy.get_action_and_value(
        #                         obs, predicted_actions
        #                     )
        return predicted_actions, predicted_logprobs, predicted_values        
                ...
                for step in range(0, self.args.num_steps):
                    ...
                    with torch.no_grad():
                        if mode == "Training":
                            ...
                            used_policies = ["internal"] * self.args.num_envs
                            
                        action, logprob, value = self.scheduler.get_action_and_value(obs=next_obs, policy_type=used_policies)
                ...
                for epoch in range(self.args.update_epochs):
                    ...
                    for start in range(0, self.args.batch_size, self.args.minibatch_size):
                        ...
                        _, newlogprob, entropy, newvalue = self.scheduler.internal_policy.get_action_and_value(
                            obs.reshape((-1,))[mb_inds], b_actions[mb_inds]
                        )
                        logratio = newlogprob - logprobs.reshape(-1)[mb_inds] # true
                        ratio = logratio.exp() # == 1 so should learn smth? # true
                        with torch.no_grad():
                            # calculate approx_kl http://joschu.net/blog/kl-approx.html
                            old_approx_kl = (-logratio).mean()
                            approx_kl = ((ratio - 1) - logratio).mean() # 0s here # f
                            ...
                            debug_ratio.extend(ratio.cpu().numpy())
                            debug_approx_kl.append(approx_kl.cpu().numpy())
                            debug_clipfracs.extend(clipfracs)
                            
                            # print(f"clipfracs {clipfracs}")
                            self.logger.log_metrics("losses/value_to_clip_mean", (ratio - 1.0).abs().mean().item(), global_step)
                        ...
                        if (
                            ...
                            print(f"breaking because approx_kl {approx_kl} > self.args.target_kl {self.args.target_kl}")
                            break
                        ...
                        # Policy loss
                        pg_loss1 = -mb_advantages * ratio # f t
